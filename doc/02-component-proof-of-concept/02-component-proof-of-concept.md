...
I decided to move forward with the BudgetEnvelope component because it provides a clear, real-world structure for managing expenses, income, and budgets. It’s simple enough to implement within one semester but still demonstrates key programming logic such as validation, balance checking, and overspending detection. 

Based on the feedback from the brainstorming project, I added a hasCategory() method to safely check whether a category exists before performing any operations. I also refined the isOverspent() logic to handle missing categories gracefully. This design will allow me to clearly demonstrate both kernel and secondary functionality through data handling and conditional logic.
...
I've created BudgetEnvelop.java in src
